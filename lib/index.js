...
var SpriteMagic = function () {
    ...
    _createClass(SpriteMagic, [{
        ...
        value: function createMixins() {
            ...
            // sprite image class
            var selector = ['.' + this.context.mapName + '-sprite'].concat(selectors.map(function (image) {
            ...
            // <map>-<sprite> mixins
            var createPseudoClassMixins = function createPseudoClassMixins(basename, cb) {
                return !pseudo[basename] ? '' : ['active', 'hover', 'target'].map(function (pseudoClass) {
                    return !pseudo[basename][pseudoClass] ? '' : cb('&:' + pseudoClass + ', &.' + basename + '_' + pseudoClass + ', &.' + basename + '-' + pseudoClass, pseudo[basename][pseudoClass]);
                }).join('');
            };
            (_context$mixins = this.context.mixins).push.apply(_context$mixins, _toConsumableArray(selectors.map(function (image) {
                ...
                createPseudoClassMixins(image.name, function (selector, image) {
                    return '\n                ' + selector + ' {\n                    background-position: ' + cssValue(-image.x, 'px') + ' ' + cssValue(-image.y, 'px') + ';\n                    width: ' + image.width + 'px;\n                    height: ' + image.height + 'px;\n                }';
                }) + '\n            }'].join('');
            })));

            // <map>-sprite() mixin
            this.context.mixins.push('\n            @mixin ' + this.context.mapName + '-sprite($name) {' + selectors.map(function (image, index) {
                return '\n                ' + (index === 0 ? '@if' : '@else if') + ' $name == \'' + image.name + '\' {\n                    @include ' + _this5.context.mapName + '-' + image.name + ';\n                }';
            }).join('') + '\n            }');

            // all-<map>-sprites mixin
            this.context.mixins.push('\n            @mixin all-' + this.context.mapName + '-sprites {' + selectors.map(function (image) {
