...
var SpriteMagic = function () {
    ...
    _createClass(SpriteMagic, [{
        ...
        value: function createSpriteImage() {
            ...
            return Promise.resolve().then(function () {
                return imagemin.buffer(_this4.context.imageData, { use: [pngquant(_this4.options.pngquant)] });
            }).then(function (buf) {
                return _this4.context.imageData = buf;
            }).then(function () {
                return new Promise(function (resolve, reject) {
                    fs.outputFile(_this4.context.imagePath, _this4.context.imageData, function (err) {
        ...
        value: function createContents() {
            ...
            this.context.contents = contents.map(function (_) {
                return _ + '\n';
            }).join('').slice(1).replace(/^\x20{12}/mg, '');
        }
    }, {
        key: 'getSelectorInfo',
        ...
        value: function output() {
            ...
            var fileName = this.context.mapName + '-' + hash.substr(0, 7) + '.scss';
            var filePath = path.resolve(os.tmpdir(), 'sprite-magic-importer', fileName);

            return new Promise(function (resolve, reject) {
                fs.outputFile(filePath, _this5.context.contents, function (err) {
                    if (err) {
                        return reject(err);
                    }
