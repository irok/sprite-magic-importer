    ...
    _createClass(SpriteMagic, [{
        ...
        value: function createMixins() {
            ...
            function createPseudoMixin(image) {
                return !pseudo[image.name] ? '' : ['active', 'hover', 'target'].map(function (pseudoClass) {
                    var pseudoImage = pseudo[image.name][pseudoClass];
            ...
            (_context$mixins = this.context.mixins).push.apply(_context$mixins, _toConsumableArray(selectors.map(function (image) {
                return ['\n            @mixin ' + _this5.context.mapName + '-' + image.name + ' {\n                background-position: ' + cssValue(-image.x, 'px') + ' ' + cssValue(-image.y, 'px') + ';', createPseudoMixin(image), '\n            }'].join('');
            })));
