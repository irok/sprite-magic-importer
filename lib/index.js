    ...
    _createClass(SpriteMagic, [{
        ...
        value: function createHash() {
            ...
            var fingerprint = this.context.images.map(function (image) {
                return _this3.projectRelPath(image.filePath) + '#' + image.size;
        ...
        value: function outputSpriteImage() {
            ...
            return Promise.resolve().then(function () {
                ...
                _this6.context.imageHash = crypto.createHash('sha256').update(buf).digest('hex');
                return fs.outputFileAsync(_this6.spriteImagePath(), buf);
            }).then(function () {
                var data = JSON.stringify({ hash: _this6.context.imageHash });
